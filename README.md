# Nuclear-Channels-Monolithic
Lightweight Web API Framework using Custom Attributes and Http Listener. This is a monolithic solution made from combining all other dlls from Nuclear-Framework.
### This is the solution from which Nuget package is being generated
...
If you want a standard Json output you can decorate your class which will be return with StandardJsonMessageAttribute and ChannelMessage wont be created. Also you can decorate your ChannelMethod with StandardJsonMessageAttribute with the same result. 

```c#
[ChannelMethod]
[StandardJsonMessage]
public TestClass TestStandardJsonMessageAttributeOnMethod(string id, string name)
{
    return new TestClass { Id = id, Name = name };
}
```

Another way to achieve the same result is to call with WritePureJson of the ChannelMessageWriter

```c#
[ChannelMethod]
public void WritePureJsonWithMessageWriter(string message)
{
    ChannelMessageWriter.WritePureJson(message, Context.Response);
}
```

if you already serialized your json object you can call WritePureJsonSerialized of the ChannelMessageWriter

```c#
[ChannelMethod]
public void WritePureJsonWithMessageWriter(string message)
{
    ChannelMessageWriter.WritePureJsonSerialized(message, Context.Response);
}
```
...
```c#
    class Program
    {
        ...
        {
            IChannelServer server = ChannelServerBuilder.CreateServer();
            server.RegisterChannels(new List<string> { "Nuclear.Channels.Monolithic.Tests " });
