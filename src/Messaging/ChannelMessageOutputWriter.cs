...
namespace Nuclear.Channels.Messaging
{
    ...
    internal class ChannelMessageOutputWriter : IChannelMessageOutputWriter
    {
        ...
        public void WritePureJson(object responseObject, HttpListenerResponse response)
        {
            _msgService.WriteHttpResponseAsPureJson(responseObject, response, false);
            OnPostMessageServiceInvoked?.Invoke(this, EventArgs.Empty);
        }

        public void WritePureJsonSerialized(string jsonResponse, HttpListenerResponse response)
        {
            _msgService.WriteHttpResponseAsPureJson(jsonResponse, response, true);
            OnPostMessageServiceInvoked?.Invoke(this, EventArgs.Empty);
        }
    }
}
